...
import { Api, CountryState, State } from '../models/config/app-config';
...
export class CovidDataService {
  ...
  private api: Api;
  private countriesStates: CountryState[];

  constructor(private http: HttpClient, private appConfig: AppConfigService) {
    this.api = this.appConfig.data.api;
    this.countriesStates = this.appConfig.data.countriesStates;
  }

  getCountries(countryCodes: string[]): Observable<CovidInfo[]>{
    if(!!countryCodes)
      return this.http.get<CovidInfo[]>(this.api.countries + countryCodes.join(","));
    return of([]);
  }

  getCountry(code: string): Observable<CovidInfo>{
    if(!!code)
      return this.http.get<CovidInfo>(this.api.countries + code);
    return of();
  }
  ...
  getCountrySummary(code: string, duration: number): Observable<HistoricalData[]>{
    ...
    this.http.get<HistoryRaw>(`${this.api.historical}${code}?lastdays=${duration}`)
  ...
  private getUsSummary(code: string): Observable<HistoricalData[]>{
    ...
    return this.http.get<UsStateResult[]>(this.api.states)
    .pipe(map(d => d.map(r => {
  ...
  private getEveryProvince(code: string, d: HistoryRaw, duration: number){
    let pp = [...d.province];
        let p = pp.join(",");
        this.http.get<HistoryRaw[]>(`${this.api.historical}${code}/${p}?lastdays=${duration}`)
  ...
  private hasStates(countryCode: string): boolean{
    let country = this.countriesStates.find(x=> x.abbreviation == countryCode);
    return !!country;
  }
  ...
  private getStateMap(countryCode: string): Map<string, State>{
    let c = this.countriesStates.find(x=> x.abbreviation == countryCode);
